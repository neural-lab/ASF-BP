...
import random 
import pdb
from dvscifar10_dataloader import DVSCifar10
...
parser = argparse.ArgumentParser(description='PyTorch DVSCIFAR10 Training')
parser.add_argument('--dataset', default='DVSCIFAR10', type=str, help='dataset = [MNIST]')
...
parser.add_argument('--steps', default=100, type=int,
                    metavar='N', help='steps (default: 100)')
...
parser.add_argument('--vth', default=1, type=float,
                    metavar='vth', help='vth (default: 1)')
parser.add_argument('--avgtimes', default=9, type=float,
                    metavar='avgtimes', help='avgtimes (default: 9)')
parser.add_argument('--leak', default=1, type=float,
                    metavar='leak', help='leaky parameter (default: 1)')
...
dvscifar10_path = '/data/diospada/dvs-cifar10/dvs-cifar10'
sign = 1
...
def main():
    global args, best_prec1, batch_size, device_num,sign
    seed1 = 44
    seed2 = 85
    seed3 = 63
    ...
    print("rand num: %d %d %d"%(seed1,seed2,seed3))
    ...
    train_data = DVSCifar10(dvscifar10_path, train=True, transform=transforms.ToTensor(), target_transform=None,steps=steps, count=args.count, per=args.per)
    train_loader = torch.utils.data.DataLoader(train_data,
                                               batch_size=args.batch_size, shuffle=True,
                                               num_workers=args.workers,
                                               pin_memory=True)
    time_read1 = time.time() - time_read1
    print('read data of testing takes %dh %dmin'%(time_read1/3600, (time_read1%3600)/60))

    time_read1 = time.time()
    val_data = DVSCifar10(dvscifar10_path, train=False,transform=transforms.ToTensor(),steps=steps, count=args.count, per=args.per)
    ...
    for epoch in range(args.start_epoch, args.epochs):
        if epoch % 5 == 0 and args.hz < args.epochs:
            sign = 1
        else:
            sign = 0
        ...
        # train for one epoch
        train(train_loader, model, criterion, criterion_en, optimizer, epoch, time_steps=steps, leak=args.leak)

        # evaluate on validation set
        modeltest = model
        prec1 = validate(val_loader, modeltest, criterion, criterion_en, time_steps=steps, leak=args.leak)
...
def save_checkpoint(state, is_best, filename='checkpointT1_dvscifar10_v7.pth.tar'):
    torch.save(state, filename)
    if is_best:
        shutil.copyfile(filename, 'model_bestT1_dvscifar10_v7.pth.tar')
...
def adjust_learning_rate(optimizer, epoch):
    ...
    for param_group in optimizer.param_groups:
        if epoch >= change5:
            param_group['lr'] = 0.5 * 0.5 * 0.2 * 0.2 * 0.2 * lr

        elif epoch >= change4:
            param_group['lr'] = 0.5 * 0.2 * 0.2 * 0.2 * lr
...
class CNNModel(nn.Module):
    def __init__(self):
        ...
        self.cnn11 = nn.Conv2d(in_channels=2, out_channels=64, kernel_size=3, stride=1, padding=1, bias=False)
        ...
        self.cnn23 = nn.Conv2d(in_channels=128, out_channels=256, kernel_size=3, stride=1, padding=1, bias=False)
        #self.BN2 = nn.BatchNorm2d(128)

        self.avgpool2 = nn.MaxPool2d(kernel_size=2)

        self.fc0 = nn.Linear(256 * 10 * 10, 1024, bias=False)
    ...
    def forward(self, input, steps=100, l=1):
        ...
        mem_11 = torch.zeros(input.size(0), 64, 42, 42, device = input.device)

        mem_12 = torch.zeros(input.size(0), 64, 42, 42, device = input.device)

        mem_1s = torch.zeros(input.size(0), 64, 21, 21, device = input.device)

        mem_21 = torch.zeros(input.size(0), 128, 21, 21, device = input.device)
        mem_22 = torch.zeros(input.size(0), 128, 21, 21, device = input.device)
        mem_23 = torch.zeros(input.size(0), 128*2, 21, 21, device = input.device)

        membrane_f0 = torch.zeros(input.size(0), 1024, device = input.device)
        
        Total_input = torch.zeros(input.size(0), 2, 42, 42, device = input.device)

        Total_11_output = torch.zeros(input.size(0), 64, 42, 42, device = input.device)
        IF_11_in = torch.zeros(input.size(0), 64, 42, 42, device = input.device)

        Total_12_output = torch.zeros(input.size(0), 64, 42, 42, device = input.device)
        IF_12_in = torch.zeros(input.size(0), 64, 42, 42, device = input.device)

        Total_p1_output = torch.zeros(input.size(0), 64, 21, 21,device = input.device)
        IF_p1_in = torch.zeros(input.size(0), 64, 21, 21, device = input.device)

        Total_21_output = torch.zeros(input.size(0), 128, 21, 21, device = input.device)
        IF_21_in = torch.zeros(input.size(0), 128, 21, 21, device = input.device)

        Total_22_output = torch.zeros(input.size(0), 128, 21, 21, device = input.device)
        IF_22_in = torch.zeros(input.size(0), 128, 21, 21, device = input.device)
        
        Total_23_output = torch.zeros(input.size(0), 128*2, 21, 21, device = input.device)
        IF_23_in = torch.zeros(input.size(0), 128*2, 21, 21, device = input.device)
        ...
        with torch.no_grad():
            for i in range(steps * args.repeat):
                # Get input using frames
                eventframe_input = input[:, :, :, :, i % steps].float()
                eventframe_input = args.avgtimes * self.maxpool4(eventframe_input)
                Total_input = Total_input + eventframe_input

                # convolutional Layer
                in_layer = self.cnn11(eventframe_input)
    ...
    def tst(self, input, steps=100, l=1):
        ...
        for i in range(steps * args.repeat):
            ...
            # convolutional Layer
            mem_11 = mem_11 + self.cnn11(eventframe_input)
            mem_11, out = LIF_sNeuron(mem_11, self.cnn11.threshold, l, i)
