...
def main():
    ...
    for epoch in range(args.start_epoch, args.epochs):
        ...
        start_end = time.time()
        # train for one epoch
        prec1_tr = train(train_loader, model, criterion, criterion_en, optimizer, epoch, time_steps=100, leak=1)

        # evaluate on validation set
        modeltest = model.module
        prec1 = validate(val_loader, modeltest, criterion, criterion_en, time_steps=100, leak=1)
...
class CNNModel(nn.Module):
    ...
    def tst(self, input, steps=100, l=1):
        ...
        for i in range(steps):
            ...
            mem_1, out = LIF_sNeuron1(mem_1, self.cnn1.threshold, l, i)

            # pooling Layer
            mem_1s = mem_1s + self.avgpool1(out)
            mem_1s, out = Pooling_sNeuron1(mem_1s, 0.75, i)

            # convolutional Layer
            mem_2 = mem_2 + self.cnn2(out)
            mem_2, out = LIF_sNeuron1(mem_2, self.cnn1.threshold, l, i)

            # pooling Layer
            mem_2s = mem_2s + self.avgpool2(out)
            mem_2s, out = Pooling_sNeuron1(mem_2s, 0.75, i)
